...
create-react-app 是 facebook 提供的脚手架，用于快速构建基本的 React 单页项目。它集成非常众多且好用的官方工具，且一般情况下不需要任何配置。基本上是我目前的首选脚手架。
...
通过该命令会启动开发者模式，在这个模式下，当修改代码的时候，会自动刷新页面并将你写的最新代码反应到页面上。如果发生任何错误，也可以在控制台看到错误提示，就像下面这样：
...
当然，create-react-app 也帮我们集成了测试功能，默认情况下，测试会自动对最后一次提交的代码进行测试。但是在国内互联网环境下，我们是很少写测试的，除非由测试人员来写，或者拥有足够的开发时间，不过这一般是不可能的。
    `,
  },
  {
    type: 'section',
...
这一点是很奇葩的，官方居然并不是很提倡大家使用 CSS 预处理。而官方文档也仅仅提供了 sass 的使用方法。这是很让人苦恼的。毕竟 Less 易学易用。在国内还是非常普遍的。

所以我自己写了一个工具，可以保证既不影响 react-scripts 升级的前提下，又可以满足大家对于 Less 的需求
[工具](https://github.com/ryouaki/react-less-watcher)
